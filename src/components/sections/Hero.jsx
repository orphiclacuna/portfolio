...
export default function Hero() {
    ...
    const headingOpacity = useTransform(scrollYProgress, [0, 0.9, 1], [1, 1, 0])
    ...
    return (
        <div ref={containerRef} id="home" className="relative h-[150vh]">
            <section className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-6">
                        ...
                        <motion.div style={{ opacity }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 7 }} className="text-2xl sm:text-3xl md:text-4xl text-text-secondary font-medium">
                            <TypingEffect phrases={phrases} />
                        </motion.div>
                        <motion.p style={{ opacity }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 7.2 }} className="max-w-2xl mx-auto text-lg text-text-muted">
                            Building beautiful and functional web experiences with passion and creativity
                        </motion.p>
                        <motion.div style={{ opacity }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 8 }} className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8 mb-16">
                ...
                <motion.div style={{ opacity }} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.4 }} className="absolute right-8 top-1/2 -translate-y-1/2 flex flex-col gap-4">
                    {socialLinks.map((social, index) => (
                        ...
                        >
                            ...
                            <div className="p-3 text-white hover:text-[#da3e87] transition-colors">
